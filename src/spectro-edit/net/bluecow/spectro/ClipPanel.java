 ...
 * <p>
 * This component fires undoable edit events regarding its region selection
 * state, so it would be wise to attach it to the same undo manager that the
 * clip is attached to.
 */
public class ClipPanel extends JPanel implements Scrollable {
    ...
    /**
     * A rectangular frame that some tools use as a bounding box for the changes
     * they make. The region feature can be turned on and off.
     */
    private Rectangle region;

    /**
     * The place where the region was last time it was repainted. Starts off as
     * null, and gets reset to null whenever region selection is turned off.
     */
    ...
    /**
     * Gets set to true while an undo is in progress. When true, undo events are
     * not fired.
     */
    ...
    /**
     * Creates a new ClipPanel component for the given clip. Also attaches to
     * the given player thread, and will show a visual indication of playback
    ...
    /**
     * Converts the given rectangle (which is in screen coordinates) to
     * co-ordinates that work with clips and their frames. The x value will be
     * the corresponding frame number within the clip, and the y value will be
     * the corresponding index within the frame's spectral data.
     * <p>
     * This is the inverse operation to {@link #toScreenCoords(Point)}.
     * 
     * <pre>
     ...
     * ValueColorizer and your settings have changed. Image updates due to clip
     * data change events are handled automatically. TODO have colorizers fire
     * change events so this method can be private again
     * 
     * @param region
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (clipBounds != null) {
            g2.drawImage(img, clipBounds.x, clipBounds.y, clipBounds.x + clipBounds.width, clipBounds.y + clipBounds.height, clipBounds.x, clipBounds.y, clipBounds.x
                    + clipBounds.width, clipBounds.y + clipBounds.height, Color.BLACK, null);
        } else {
     ...
     * be necessary to call this method directly; use
     * {@link #setRegion(Rectangle)}.
     */
     ...
     * 
     * @param r
     *            The new location and size for the selected region.
     */
    ...
    /**
     * Handles mouse events on this panel for purposes of creating, moving, and
     * resizing the region.
     */
    private class RegionMouseHandler implements MouseMotionListener, MouseListener {
        ...
        /**
         * The offset from the region's (x,y) origin that the mouse should stay
         * at when the region is being moved.
         */
        Point moveHandle;
    ...
    /**
     * Creates a copy of the given rectangle with nonnegative width and height.
     * The new rectangle's actual geometry is the same as the given rectangle's.
     * 
     * @param rect
     *            The source rectangle. This rectangle will not be changed as a
     *            result of this call. You can pass in null, which results in a
     *            null return value.
    ...
    /**
     * The header component for this clip panel. Automatically installs when
     * this panel is in a JScrollPane.
     */
    private class ClipPositionHeader extends JPanel implements PlaybackPositionListener {
        ...
        /**
         * Calculates the correct x-coordinate to position the playback
         * indicator at, based on the clip's settings and the current playback
         * position.
         */
        ...
        /**
         * Repositions the playback thread to the frame corresponding to the
