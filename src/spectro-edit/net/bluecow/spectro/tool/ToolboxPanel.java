...
public class ToolboxPanel {
  ...
  /**
   * The clip panel we're modifying settings for. This is meant to be the same clipPanel that
   * belongs to {@link #session}.
   */
  ...
  public ToolboxPanel(SpectroEditSession session) {
    ...
    JRadioButton paintbrushToolButton =
        new ToolButton(new PaintbrushTool(), "paintbrush", toolButtonGroup);
    toolButtonPanel.add(paintbrushToolButton);
    paintbrushToolButton.addActionListener(actionHandler);

    JRadioButton regionScaleToolButton =
        new ToolButton(new RegionScaleTool(), "contrast_change", toolButtonGroup); // TODO better
                                                                                   // icon
    toolButtonPanel.add(regionScaleToolButton);
    regionScaleToolButton.addActionListener(actionHandler);

    JRadioButton regionThresholdToolButton =
        new ToolButton(new RegionThresholdTool(), "threshold", toolButtonGroup); // TODO better icon
    toolButtonPanel.add(regionThresholdToolButton);
    regionThresholdToolButton.addActionListener(actionHandler);

    JRadioButton flipToolButton =
        new ToolButton(new RegionFlipTool(), "shape_flip_vertical", toolButtonGroup); // TODO better
                                                                                      // icon
    toolButtonPanel.add(flipToolButton);
    flipToolButton.addActionListener(actionHandler);

    JRadioButton magnifyToolButton =
        new ToolButton(new MagnifyTool(), "magnifier", toolButtonGroup); // TODO better icon
