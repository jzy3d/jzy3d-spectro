...
/**
 * Simple extension to JSlider that provides a nonlinear (curved) mapping of knob position to value.
 * Also supports floating point values rather than simply integers. To get the curved value, call
 * the {@link #getCurvedValue()} method. The {@link #getValue()} method returns a raw linear, scaled
 * value.
 */
public class CurvedSlider extends JSlider {
   ...
   * @param curveSeverity the amount of curviness the slider's range should have. Larger values mean
   *        finer control over low values and a sharper transition to big steps toward the top of
   *        the slider's range. Useful values lie between 1.0 (linear) and 4.0 (very curved).
   */
  public CurvedSlider(double min, double max, double curviness) {
    ...
    // c is (max^(1/b))/resolution -- this ensures the max value is max, regardless of the value of
    // b or resolution
    // y is getCurvedValue()
    // x is getValue()
  ...
  /**
   * Returns the current slider value after applying the curve and resolution settings.
   */
