...
/**
 * Tool that sets any data point that was over a threshold amount to 0, leaving all other data
 * points at their original values. This is a nice way to remove some sound that is significantly
 * louder than the background noise, leaving only what was in the background.
 */
public class RegionThresholdTool implements Tool {
  ...
  /**
   * The data we've captured. This is what the data in the selected region looked like when we
   * started, and it's what we scale when the slider moves. TODO if we could ask Clip for the undo
   * data of its current in-progress edit, we would not need this
   */
  ...
  /**
   * If checked, this tool imposes an upper threshold (removes loud parts); if false it is a lower
   * threshold (removes quiet parts).
   */
