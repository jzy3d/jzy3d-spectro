...
public abstract class AbstractRegionalTool implements Tool {
  ...
  /**
   * Sets the clip panel into region mode and installs a region change listener on it. Also calls
   * {@link #regionChanged(Rectangle)} with the initial region, which could be null. If you override
   * this method, be sure to invoke this one with a super.activate(session) in your override.
   */
  @Override
  ...
  /**
   * Removes the region change listener from this tool. If you override this method, be sure to
   * invoke this one with a super.deactivate(session) in your override.
   */
  @Override
  ...
  /**
   * This method is called whenever the tool is active and the current region of the clip has been
   * moved or resized. You can override it if you need to take action when the selected region has
   * changed. If you override this method, be sure to call super.regionChanged() in order to update
   * the {@link #region} rectangle.
   * 
   * @param region The new bounds of the selected clip region.
   */
  ...
  /**
   * Handles change events from the clip and relays them to the various protected update methods.
   */
