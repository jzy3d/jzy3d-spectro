...
/**
 * The central point of coordination for all the parts that work together to provide a GUI editing
 * environment for an audio file.
 * <p>
...
public class SpectroEditSession {
  ...
  /**
   * The undo manager that keeps track of changes in this session, including the clip data and the
   * state of various tools.
   */
  ...
  protected SpectroEditSession(Clip c) throws LineUnavailableException {
    ...
    if (prefs.get("frameBounds", null) != null) {
      ...
      if (frameBounds.length == 4) {
        f.setBounds(Integer.parseInt(frameBounds[0]), Integer.parseInt(frameBounds[1]),
            Integer.parseInt(frameBounds[2]), Integer.parseInt(frameBounds[3]));
      }
    } else {
      f.pack();
      Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
      f.setSize(Math.min(screenSize.width - 50, f.getWidth()),
    ...
    f.addWindowListener(new WindowAdapter() {
      ...
      public void windowClosing(WindowEvent e) {
        try {
          prefs.put("frameBounds",
              String.format("%d,%d,%d,%d", f.getX(), f.getY(), f.getWidth(), f.getHeight()));
          prefs.flush();
  ...
  /**
   * Creates a new session with a GUI for editing a Clip. The Clip's contents will be initialized to
   * correspond with the given file, which should be in a PCM-encoded WAV or AIFF file.
   * 
  ...
  /**
   * Launches the Spectro-Edit application by prompting the user for a file, then creating a new
   * session for editing that file.
   * 
  ...
  public static void main(String[] args) throws Exception {
    ...
    LogManager.getLogManager()
        .readConfiguration(SpectroEditSession.class.getResourceAsStream("LogManager.properties"));
